    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-12 animate-fade-in">
      ...
      <div className={`bg-white rounded-[2rem] p-8 shadow-xl border-t-4 ${currentApp.border.replace('border-', 'border-t-')}`}>
        ...
        <div className="space-y-6">
           ...
           {/* Specialized Controls for Sheets */}
           {activeApp === 'sheets' && (
             <div className="flex gap-4 mb-2">
               <button 
                 onClick={() => setSheetMode('formula')}
                 className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${sheetMode === 'formula' ? 'bg-green-100 text-green-700' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
               >
                 <i className="fa-solid fa-calculator mr-2"></i> Formula Generator
               </button>
               <button 
                 onClick={() => setSheetMode('data')}
                 className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${sheetMode === 'data' ? 'bg-green-100 text-green-700' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}
               >
                 <i className="fa-solid fa-table mr-2"></i> Dummy Data
               </button>
             </div>
           )}
           ...
           <div>
              ...
              <textarea
                ...
                style={{
                    focusRingColor: activeApp === 'docs' ? '#2563eb' : activeApp === 'sheets' ? '#16a34a' : activeApp === 'slides' ? '#ea580c' : activeApp === 'mail' ? '#0ea5e9' : '#db2777'
                }}
              />
           </div>
