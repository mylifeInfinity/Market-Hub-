...
const ImageEditor: React.FC<ImageEditorProps> = ({ image, onSave, onCancel }) => {
  ...
  const extractColors = () => {
    ...
    try {
        ...
        // Sample pixels (step of 1000 for performance)
  ...
  // Draw the preview whenever settings change
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    ...
    img.onload = () => {
      ...
      // Limit canvas size for performance/display, but keep aspect
      const maxWidth = 800;
      ...
      // Combine Filter + Brightness/Contrast
      ...
      // Apply Overlay if exists (e.g. for Tinting)
      ...
      // 3. Draw Crop Overlay (Visual Guide)
      if (cropRatio) {
        ...
        ctx.fillRect(0, y + cropH, canvas.width, canvas.height - (y + cropH)); // Bottom
        ctx.fillRect(0, y, x, cropH); // Left
        ctx.fillRect(x + cropW, y, canvas.width - (x + cropW), cropH); // Right
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                <div className="flex-1 bg-slate-900 flex flex-col items-center justify-center p-8 overflow-auto relative">
                    ...
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-3">
                        ...
                        >
                            <i className="fa-solid fa-eye-dropper"></i> {extractedPalette.length > 0 ? 'Refresh Palette' : 'Extract Palette'}
                        </button>
                    </div>
                </div>
                ...
                <div className="w-full lg:w-80 bg-white border-l border-slate-100 p-6 overflow-y-auto custom-scrollbar flex flex-col gap-8">
                    ...
                    <div className="space-y-4">
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
                            <i className="fa-solid fa-wand-magic-sparkles"></i> Filters & Styles
                        </h4>
                        <div className="grid grid-cols-2 gap-2">
                            {FILTER_PRESETS.map((f, i) => (
                                <button
                                    ...
                                    className={`py-2 px-3 rounded-lg text-xs font-bold border transition-all text-left ${activeFilter.name === f.name ? 'bg-pink-50 border-pink-200 text-pink-700' : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100'}`}
                                >
                                    {f.name}
                                </button>
                            ))}
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={() => { setBrightness(100); setContrast(100); setActiveFilter(FILTER_PRESETS[0]); }} className="text-xs text-pink-600 font-bold hover:underline">Reset All</button>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setRotation(r => r - 90)} className="flex-1 py-2 bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 hover:border-slate-300 transition-all text-slate-600">
                    ...
                    <div className="space-y-4">
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
                            <i className="fa-solid fa-crop-simple"></i> Smart Crop
                        </h4>
                        <div className="grid grid-cols-2 gap-2">
                            <button 
                                onClick={() => setCropRatio(null)} 
                                className={`py-2 rounded-xl text-xs font-bold border transition-all ${cropRatio === null ? 'bg-pink-50 border-pink-200 text-pink-700' : 'bg-white border-slate-200 text-slate-600'}`}
                            >
                                Original
                            </button>
                            <button 
                                onClick={() => setCropRatio(1)} 
                                className={`py-2 rounded-xl text-xs font-bold border transition-all ${cropRatio === 1 ? 'bg-pink-50 border-pink-200 text-pink-700' : 'bg-white border-slate-200 text-slate-600'}`}
                            >
                                Square (1:1)
                            </button>
                            <button 
                                onClick={() => setCropRatio(16/9)} 
                                className={`py-2 rounded-xl text-xs font-bold border transition-all ${cropRatio === 16/9 ? 'bg-pink-50 border-pink-200 text-pink-700' : 'bg-white border-slate-200 text-slate-600'}`}
                            >
                                Landscape (16:9)
                            </button>
                            <button 
                                onClick={() => setCropRatio(4/3)} 
                                className={`py-2 rounded-xl text-xs font-bold border transition-all ${cropRatio === 4/3 ? 'bg-pink-50 border-pink-200 text-pink-700' : 'bg-white border-slate-200 text-slate-600'}`}
                            >
                                Portrait (4:3)
                            </button>
                        </div>
...
const ImageGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Append color style to prompt if selected
  ...
  const filteredHistory = history.filter(img => {
      ...
      if (filterDate === 'today') {
         ...
         matchesDate = imgDate >= oneMonthAgo;
      }
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {STYLE_INSPIRATIONS.map((style) => (
                ...
                >
                  ...
                  <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-white text-xs bg-black/50 px-2 py-1 rounded-full">
                    Use Prompt
                  </div>
                </div>
              ))}
            ...
            <div className="flex flex-col gap-4">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative">
                        ...
                        placeholder="Describe the image you want to create (e.g., A futuristic city in a glass bottle)..."
                    ...
                    <button
                        ...
                        className={`
                        px-8 py-4 rounded-xl font-medium text-white shadow-lg shadow-pink-500/30 transition-all whitespace-nowrap
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white/50 p-4 rounded-2xl border border-white/50 backdrop-blur-sm animate-fade-in">
                <div className="relative w-full md:w-96">
                    <input 
                        ...
                        className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 focus:border-pink-500 focus:ring-2 focus:ring-pink-200 outline-none text-sm transition-all"
                    />
            ...
            filteredHistory.length > 0 ? (
                ...
                <div className="text-center py-20 bg-white/30 rounded-3xl border border-dashed border-slate-300 backdrop-blur-sm">
                    <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <i className="fa-solid fa-filter-circle-xmark text-2xl text-slate-400"></i>
                    </div>
                    <h3 className="text-lg font-bold text-slate-700 mb-1">No matches found</h3>
                    <p className="text-slate-500 font-medium mb-4">Try adjusting your search or date filters.</p>
                    <button 
