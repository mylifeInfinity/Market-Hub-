...
import { VFX_SOFTWARE, VFX_LEARNING } from '../data/toolsData';
...
const VfxHub: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-12 animate-fade-in space-y-20">
      ...
      <div className="relative rounded-[3rem] overflow-hidden bg-slate-900 text-white p-10 md:p-14 text-center shadow-2xl border border-slate-800">
         <div className="absolute top-0 right-0 w-96 h-96 bg-orange-500/20 rounded-full blur-[120px] -mr-20 -mt-20"></div>
         <div className="absolute bottom-0 left-0 w-96 h-96 bg-purple-500/20 rounded-full blur-[120px] -ml-20 -mb-20"></div>
         
         <div className="relative z-10 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-xs font-bold text-orange-300 uppercase tracking-widest mb-6">
               <i className="fa-solid fa-wand-magic-sparkles"></i> Visual Effects HQ
            </div>
            <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight">
               VFX Studio
            </h1>
            <p className="text-lg md:text-xl text-slate-400 font-medium leading-relaxed max-w-2xl mx-auto">
               The ultimate resource for motion designers and VFX artists. Generate shot breakdowns, access industry-standard software, and learn from the pros.
            </p>
         </div>
      </div>
      ...
      <section>
         <div className="flex items-center gap-4 mb-8 px-4">
           <div className="w-12 h-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-xl shadow-lg shadow-indigo-500/20">
              <i className="fa-solid fa-microchip"></i>
           </div>
           <div>
             <h2 className="text-3xl font-bold text-slate-900">AI VFX Assistant</h2>
             <p className="text-slate-500">Accelerate your pre-production workflow.</p>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="space-y-4">
              <button onClick={() => { setActiveAction('breakdown'); setInput(''); setOutput(''); }} className={`w-full text-left p-4 rounded-2xl border transition-all flex items-center gap-4 ${activeAction === 'breakdown' ? 'bg-white border-orange-500 shadow-md ring-1 ring-orange-500/20' : 'bg-white/60 border-slate-200 hover:bg-white'}`}>
              ...
              <button onClick={() => { setActiveAction('texture_prompt'); setInput(''); setOutput(''); }} className={`w-full text-left p-4 rounded-2xl border transition-all flex items-center gap-4 ${activeAction === 'texture_prompt' ? 'bg-white border-purple-500 shadow-md ring-1 ring-purple-500/20' : 'bg-white/60 border-slate-200 hover:bg-white'}`}>
              ...
              <button onClick={() => { setActiveAction('script_to_shot'); setInput(''); setOutput(''); }} className={`w-full text-left p-4 rounded-2xl border transition-all flex items-center gap-4 ${activeAction === 'script_to_shot' ? 'bg-white border-blue-500 shadow-md ring-1 ring-blue-500/20' : 'bg-white/60 border-slate-200 hover:bg-white'}`}>
           ...
           <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100">
                 ...
                 <textarea 
                    ...
                    className="w-full px-5 py-4 rounded-xl bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all h-32 resize-none mb-4"
              ...
              {output && (
                 <div className="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-100 animate-fade-in-up">
                    <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-4">
                       <h3 className="font-bold text-lg text-slate-800">AI Analysis Result</h3>
      ...
      <section>
         <div className="flex items-center gap-4 mb-8 px-4">
           ...
           <div>
             <h2 className="text-3xl font-bold text-slate-900">Popular VFX Software</h2>
             <p className="text-slate-500">Industry standard tools and suites.</p>
           </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           {VFX_SOFTWARE.map((soft: any, idx) => (
              <div key={idx} className="group bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col relative overflow-hidden">
             ...
             <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-10">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-indigo-200">Curated learning resources.</p>
                   </div>
                </div>
             </div>
              ...
              {VFX_LEARNING.map((learn, idx) => (
                 <a key={idx} href={learn.url} target="_blank" rel="noreferrer" className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10 hover:bg-white/20 transition-all flex items-center gap-4 group">
                    ...
                    <div>
                       ...
                       <p className="text-xs text-indigo-200 font-medium uppercase tracking-wide">{learn.topic}</p>
                    </div>
                 </a>
              ))}
           </div>
