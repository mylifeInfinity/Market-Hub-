...
type TabCategory = 'All' | 'Pro Software' | 'Online Editors' | 'AI Tools' | 'Open Source' | 'Mobile Apps';

const VideoEditingHub: React.FC = () => {
  ...
  const categories: TabCategory[] = ['All', 'Pro Software', 'Online Editors', 'AI Tools', 'Open Source', 'Mobile Apps'];

  return (
    <div className="max-w-7xl mx-auto pb-12 animate-fade-in space-y-16">
      
      {/* Hero Header */}
      <div className="relative rounded-[3rem] overflow-hidden bg-gradient-to-r from-red-900 to-slate-900 text-white p-10 md:p-14 text-center shadow-2xl border border-red-900/50">
         <div className="absolute top-0 right-0 w-96 h-96 bg-red-600/20 rounded-full blur-[120px] -mr-20 -mt-20 animate-pulse"></div>
         <div className="absolute bottom-0 left-0 w-96 h-96 bg-orange-600/20 rounded-full blur-[120px] -ml-20 -mb-20"></div>
         
         <div className="relative z-10 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-xs font-bold text-red-300 uppercase tracking-widest mb-6">
               <i className="fa-solid fa-clapperboard"></i> Production Suite
            </div>
            <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight">
               Video Editing Hub
            </h1>
            <p className="text-lg md:text-xl text-slate-300 font-medium leading-relaxed max-w-2xl mx-auto">
               The central station for video creators. Plan your content with AI, find the best editing software, and access powerful online tools.
            </p>
         </div>
      </div>
               ...
               <div key={idx} className="group bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col relative overflow-hidden animate-fade-in-up">
                  ...
                  <div className="mb-4">
                     ...
                     <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                        tool.category === 'AI Tool' ? 'bg-purple-50 text-purple-600' : 'bg-slate-100 text-slate-500'
                     }`}>
                        {tool.category}
                     </span>
                  </div>
                  ...
                  <a href={tool.url} target="_blank" rel="noreferrer" className="w-full py-3 rounded-xl bg-slate-50 text-slate-900 font-bold text-sm hover:bg-slate-900 hover:text-white transition-all text-center flex items-center justify-center gap-2 group-hover:shadow-lg">
                     {tool.category === 'AI Tool' ? 'Try AI Tool' : 'Get Software'} <i className="fa-solid fa-arrow-up-right-from-square text-xs"></i>
                  </a>
               </div>
            ))}
...
export default VideoEditingHub;
